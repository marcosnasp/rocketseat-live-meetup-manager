...
public class RegistrationControllerTest {
	...
	public void createRegistrationTest() throws Exception {
		...
		Registration savedRegistration = Registration.builder().id(101).name("Ana Neri")
				.dateOfRegistration("10/10/2021").registration("001").build();
		...
		MockHttpServletRequestBuilder request = MockMvcRequestBuilders.post(REGISTRATION_API)
				.contentType(MediaType.APPLICATION_JSON).accept(MediaType.APPLICATION_JSON).content(json);

		// verificacao, assert....
		mockMvc.perform(request).andExpect(status().isCreated()).andExpect(jsonPath("id").value(101))
	...
	private RegistrationDTO createNewRegistration() {
		return RegistrationDTO.builder().id(101).name("Ana Neri").dateOfRegistration("10/10/2021").registration("001")
				.build();
	}
}
